...
**Your Answers here:**  
1. The 6 first components explain 52.29% of the variance in this set of data.  

2. We can find the weights in the last table: the heighest weight for component 1 are "5yr T-Notes US" and "10yr T-Notes", both weighing 0.93   

3. We can identify the 10 first components as follows:  
Component 1 = Bonds  
Component 2 = Currencies   
Component 3 = European Indices   
Component 4 = US Indices   
Component 5 = European Futures   
Component 6 = Oil 
Component 7 = Metals 
Component 8 = Grains  
Component 9 = Rare Metals 
Component 10 = East Asian Indices   

4. New Models
a) the pre-crisis bull market years (e.g. only using the data between November 1, 2002, and October 1, 2007)?    

```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
PREcrisis_data <- futures_data[as.Date(rownames(futures_data)) > "2002-11-01" & as.Date(rownames(futures_data)) < "2007-10-01", ]
...
b) the financial crisis years (e.g. only using the data between October 1, 2007 and  March 1, 2009), (Hint: you can select subsets of the data using for example the command `crisis_data = futures_data[as.Date(rownames(futures_data)) > "2007-10-01" & as.Date(rownames(futures_data)) < "2009-03-01", ])  

```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
FINcrisis_data <- futures_data[as.Date(rownames(futures_data)) > "2007-10-01" & as.Date(rownames(futures_data)) < "2009-03-01", ]
Variance_Explained_Table_results<-PCA(FINcrisis_data, graph=FALSE)
Variance_Explained_Table<-cbind(paste("component",1:ncol(FINcrisis_data),sep=" "),Variance_Explained_Table_results$eig)
Variance_Explained_Table<-as.data.frame(Variance_Explained_Table)
colnames(Variance_Explained_Table)<-c("Component","Eigenvalue", "Percentage_of_explained_variance", "Cumulative_percentage_of_explained_variance")
```
